 ...
 */
public class SingletonImportServlet extends HttpServlet {
  ...
  private static Map<Class<?>, String> queryFilterFragmentMap =
      new ImmutableMapBuilder<Class<?>, String>()
          .put(LivingStoryEntity.class, "id")
          .put(AngleEntity.class, "lspId")
          .put(BaseAtomEntityImpl.class, "livingStoryId")
          .getMap();
  ...
  private static boolean isLspSpecificImport;
  ...
  private void setUp() {
    ...
    isLspSpecificImport = inputData.has(SpecificDataExportServlet.LSP_URLS_PARAM);
    JSONArray lspUrls = null;
    if (isLspSpecificImport) {
      try {
        lspUrls = inputData.getJSONArray(SpecificDataExportServlet.LSP_URLS_PARAM);
      } catch (JSONException ex) {
        throw new RuntimeException(ex);
      }
    }
    ...
    workQueue.add(isLspSpecificImport
        ? new DeleteSpecificDataFunction(lspUrls)
        : new DeleteAllDataFunction());
  ...
  private class DeleteAllDataFunction implements Function<Void, Boolean> {
    public Boolean apply(Void ignore) {
      ...
      for (Class<? extends JSONSerializable> entityClass
          : DataExportServlet.EXPORT_ENTITY_CLASSES) {
  ...
  /**
   * Deletes all data specific to lsps with the given urls.
   */
  private class DeleteSpecificDataFunction implements Function<Void, Boolean> {
    private boolean isSetUp = false;
    private Set<String> lspUrls = Sets.newHashSet();
    private List<Long> lspIds = Lists.newArrayList();
    private PersistenceManager pm = pms.get(0);
    
    public DeleteSpecificDataFunction(JSONArray lspUrlsJSON) {
      try {
        for (int i = 0; i < lspUrlsJSON.length(); i++) {
          lspUrls.add(lspUrlsJSON.getString(i));
        }
      } catch (JSONException ex) {
        throw new RuntimeException(ex);
      }
    ...
    public Boolean apply(Void ignore) {
      ...
      // delete all data related to these lsps.
      while (!lspIds.isEmpty()) {
        long lspId = lspIds.get(0);
        for (Class<? extends HasSerializableLspId> entityClass
            : SpecificDataExportServlet.EXPORT_LSP_ENTITY_CLASSES) {
          String queryFilterFragment = queryFilterFragmentMap.get(entityClass);

          Query query = pm.newQuery(entityClass);
          query.setFilter(queryFilterFragment + " == lspIdParam");
          query.declareParameters("long lspIdParam");
          try {
            @SuppressWarnings("unchecked")
            List<HasSerializableLspId> results = (List<HasSerializableLspId>) query.execute(lspId);
            for (HasSerializableLspId result : results) {
              if (timeout()) {
                return true;
              }
    ...
    private void setUp() {
      // At this stage, we don't expect to have more than a dozen or so living stories,
      // so just pull the entire extent to map urls to ids, instead of executing a query
      // for each requested lsp url.
      Extent<LivingStoryEntity> livingStories = pm.getExtent(LivingStoryEntity.class);

      for (LivingStoryEntity lsp : livingStories) {
        if (lspUrls.contains(lsp.getUrl())) {
          lspIds.add(lsp.getId());
        }
      }
      isSetUp = true;
    }
