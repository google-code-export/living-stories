...
/**
 * Basic preview implementation for a player atom.
 * 
...
public class BasePlayerPreview extends Composite {
  ...
  private PlayerAtom atom;

  public BasePlayerPreview(PlayerAtom atom) {
    this.atom = atom;
    
    bind();
    
    if (atom.getPhotoAtom() != null) {
      Image photoWidget = new Image();
      photoWidget.setUrl(atom.getPhotoAtom().getPreviewUrl());
      photoWidget.addStyleName("playerPhoto");
      image.add(photoWidget);
    }

    name.setText(atom.getName());
    description.add(new ContentRenderer(atom.getContent().split(Constants.BREAK_TAG)[0], false));
  }
  ...
  @UiHandler("name") void goToPlayerPage(ClickEvent e) {
    ...
    Page page = (Page) atom.renderContent(Collections.<Long>emptySet());
    HistoryManager.newToken(page, HistoryPages.PLAYER, String.valueOf(atom.getId()));
